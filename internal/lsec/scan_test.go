...
func TestScanNPMLockMetadataInventoriesLegacyNestedDependencies(t *testing.T) {
	...
	observations, diagnostics := scanNPMLockMetadata("scan-run", lockfile)
	if len(diagnostics) != 0 {
		t.Fatalf("diagnostics = %#v, want none", diagnostics)
	}
	if !scanObservationsContain(observations, "a", "1.0.0") || !scanObservationsContain(observations, "b", "2.0.0") {
		t.Fatalf("observations = %#v, want legacy nested npm dependencies", observations)
	}
}

func TestScanNPMLockMetadataInventoriesNestedScopedPackagePaths(t *testing.T) {
	lockfile := filepath.Join(t.TempDir(), "package-lock.json")
	writeFile(t, lockfile, `{
		"lockfileVersion": 3,
		"packages": {
	...
	if !scanObservationsContain(observations, "a", "1.0.0") || !scanObservationsContain(observations, "@scope/b", "2.0.0") {
		t.Fatalf("observations = %#v, want nested scoped npm dependencies", observations)
	}
}

func scanObservationsContain(observations []ScanObservation, name, version string) bool {
	for _, observation := range observations {
		if observation.Name == name && observation.Version == version {
			return true
		}
	}
	return false
